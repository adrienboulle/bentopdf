...
# Destinations (send results by HTTP): extra CSP connect-src origins, and an operator preset (JSON array)
ARG VITE_DESTINATION_HOSTS
ARG VITE_DESTINATIONS_DEFAULT
ENV VITE_DESTINATION_HOSTS=$VITE_DESTINATION_HOSTS
ENV VITE_DESTINATIONS_DEFAULT=$VITE_DESTINATIONS_DEFAULT
